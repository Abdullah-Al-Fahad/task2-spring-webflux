          ...
          if (exists) {
            return {
              requests: state.requests.map((r) => {
                ...
                const isTerminal = partial.status === 'CANCELLED' || partial.status === 'COMPLETED' || partial.status === 'FAILED';
                if (!isTerminal && partial.progress !== undefined && r.progress !== undefined && partial.progress < r.progress) {
                  return r;
                }
